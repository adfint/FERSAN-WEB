...
import application from '../RequestPaths/paths'
...
export class AuthenticationService {
    ...
    login(idEmpresa: string, password: string) {
        return this.http.post<User[]>(`${application.application.employeeCredencials}`, { idEmpresa: idEmpresa, password: password },httpOptions)
            .pipe(map(user => {
                console.log(user);
    ...
    actualizaDartosUsr(idEmpresa: string, password: string) {
        ...
        return this.http.post<User>(`${application.application.employeeCredencials}`, { idEmpresa: idEmpresa, password: password })
            .pipe(map(user => {
                 return user;
            }),
            catchError(this.handleError));
